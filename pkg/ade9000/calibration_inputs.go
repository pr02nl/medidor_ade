...
const (
	NOMINAL_INPUT_VOLTAGE     = 220 //Nominal Calibrating RMS Voltage on all phases (Vrms)
	NOMINAL_INPUT_CURRENT     = 5   //Nominal Calibrating RMS Current on all phases (Irms)
	INPUT_FREQUENCY           = 60  //Nominal Calibrating frequency. Used to calculate Omega for phase calibration.Ensure ACCMODE register is set to correct frequency
	CALIBRATION_ANGLE_DEGREES = 60  //Calibration Angle in Degrees.Used for phase calibration
	/*Transfer function*/
	/****************************************************************************************************************
	...
	/****************************************************************************************************************
	  ...
	  ****************************************************************************************************************/
	CURRENT_TRANSFER_FUNCTION = 0.00408 //The RMS voltage at the ADC input pins per input RMS current  (V/A).(2500:1-->0.00408 with default burden resistors)
	VOLTAGE_TRANSFER_FUNCTION = 0.00125 //The RMS voltage at the ADC input pins per input RMS voltage (V/V)
	/****************************************************************************************************************
										Constants: Do not change
	*****************************************************************************************************************/
	F_DSP                = 8000   //Signal update Rate
	CALIBRATION_EGY_CFG  = 0xF011 //Latch after EGYRDY. Sample based accumulation. Read with reset disabled. Accumulation enabled
	EGYACCTIME           = 0x1F3F //Accumulate for a total of 8000 (EGY_TIME+1) samples.
	CALIBRATION_ACC_TIME = 1      //if EGYACCTIME= 0x1F3F, Accumulation time is 1sec. Change this if EGYACCTIME is changed.
	/****************************************************************************************************************
	  ...
	  *****************************************************************************************************************/
	CAL_IRMS_CC   = 3.28849  // (uA/code)
	CAL_VRMS_CC   = 10.73364 // (uV/code)
	CAL_POWER_CC  = 4.73755  // (mW/code) Applicable for Active, reactive and apparent power
	CAL_ENERGY_CC = 1.34757  // (uWhr/xTHR_HI code)Applicable for Active, reactive and apparent energy
