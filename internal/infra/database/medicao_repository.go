...
func (r *MedicaoRepository) Save(medicao *entity.Medicao) error {
	stmt, err := r.Db.Prepare(`
		INSERT INTO medicao (id, medidor_id, datetime, voltage_rms_a, voltage_rms_b, voltage_rms_c, 
			...
			frequency_b, frequency_c, angle_va_vb, angle_vb_vc, angle_va_vc, angle_va_ia, angle_vb_ib, 
			angle_vc_ic, angle_ia_ib, angle_ib_ic, angle_ia_ic, temperature)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`)
	if err != nil {
		return err
	...
	defer stmt.Close()
	_, err = stmt.Exec(medicao.ID, medicao.MedidorID, medicao.DateTime, medicao.VoltageRMS_A, medicao.VoltageRMS_B,
		medicao.VoltageRMS_C, medicao.CurrentRMS_A, medicao.CurrentRMS_B, medicao.CurrentRMS_C,
		medicao.CurrentRMS_N, medicao.ActivePower_A, medicao.ActivePower_B, medicao.ActivePower_C,
		medicao.ReactivePower_A, medicao.ReactivePower_B, medicao.ReactivePower_C, medicao.ApparentPower_A,
		medicao.ApparentPower_B, medicao.ApparentPower_C, medicao.PowerFactor_A, medicao.PowerFactor_B,
		medicao.PowerFactor_C, medicao.Frequency_A, medicao.Frequency_B, medicao.Frequency_C,
		medicao.Angle_VA_VB, medicao.Angle_VB_VC, medicao.Angle_VA_VC, medicao.Angle_VA_IA,
		medicao.Angle_VB_IB, medicao.Angle_VC_IC, medicao.Angle_IA_IB, medicao.Angle_IB_IC,
		medicao.Angle_IA_IC, medicao.Temperature)
	if err != nil {
		return err
	}
	return nil
